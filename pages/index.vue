<template>
  <div>
    <h3>首页</h3>
    <button @click="upCount">更新数据 {{ count }}</button>
    <a href="/login?id=1">a标签跳转页面</a>
    <nuxt-link to="/login">标签跳转页面</nuxt-link>
    <nuxt-link :to="{ name: 'login', query: { id: 1 } }"
      >标签跳转页面 query</nuxt-link
    >
    <button @click="goLink('/login')">JS query 跳转页面</button>
    <button @click="goLinkParams('login')">JS params 跳转页面</button>
    <!-- <Tutorial /> -->
    <!-- <Tutorial /> -->
  </div>
</template>
